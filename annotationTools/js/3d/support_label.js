...
function next()// load next question, also called upon skip
{
	...
	if (img_id == MAX_Q) {
	    ...
	    $('#mainframe').attr('src', 'https://people.csail.mit.edu/hairuo/test/LabelMeAnnotationTool/tool.html?collection=LabelMe&mode=mt&threed=true&folder=example_folder&tester=true&threed_mt_mode=support_label&image=' + img_id + '&userid=' + gup("workerId")+ "&next="+next_val);// this will have to be changed to reflect labelme
	feedback = '<center>';
	pass_status = false;
	tester_init();
	}
